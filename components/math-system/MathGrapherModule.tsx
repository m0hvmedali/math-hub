            ...
            <header className="flex items-center gap-4">
                ...
                <div>
                    <h2 className="text-2xl font-black uppercase tracking-tight">
                        {language === 'ar' ? 'رسام الدوال' : 'Function Plotter'}
                    </h2>
                    <p className="text-xs text-gray-500 font-medium">
                        {language === 'ar' ? 'تمثيل بصري تفاعلي ثنائي الأبعاد' : 'Interactive 2D Visualization'}
                    </p>
                </div>
            </header>
                ...
                <div className="w-full md:w-80 space-y-4 shrink-0">
                    <div className="glass-card p-4 bg-white/5 border-white/10 rounded-2xl">
                        <label className="text-[10px] font-black uppercase tracking-widest text-gray-500 mb-2 block">
                            {language === 'ar' ? 'مدخلات الدالة' : 'Function Input'}
                        </label>
                        ...
                        <div className="space-y-2">
                             {[
                                { name: language === 'ar' ? 'موجة جيبية' : 'Sine Wave', fn: 'sin(x)' },
                                { name: language === 'ar' ? 'قطع مكافئ' : 'Parabola', fn: 'x^2' },
                                { name: language === 'ar' ? 'دالة سيجمويد' : 'Sigmoid', fn: '1/(1+exp(-x))' },
                                { name: language === 'ar' ? 'القيمة المطلقة' : 'Absolute', fn: 'abs(x)' }
                             ].map((ex, i) => (
                                <button 
                                    key={i}
                                    onClick={() => {
                    ...
                    <div className="p-4 rounded-2xl bg-brand-cyan/5 border border-brand-cyan/10">
                        <div className="flex items-center gap-2 text-brand-cyan mb-2">
                            ...
                            <span className="text-[10px] font-black uppercase tracking-widest">
                                {language === 'ar' ? 'محرك مباشر' : 'Live Engine'}
                            </span>
                        </div>
                        <p className="text-[10px] text-gray-400 leading-relaxed">
                            {language === 'ar' ? 'المعالجة بسرعة 60 إطار في الثانية باستخدام تحسينات SVG. قم بالتكبير والسحب لاستكشاف مساحة الدالة.' : 'Rendering at 60FPS using SVG optimization. Zoom and drag to explore the function space.'}
                        </p>
                    </div>
                </div>
